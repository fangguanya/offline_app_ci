...
class Audio extends Device {
  Audio(int i, int ydli) : super(i, ydli);
  // 设备类型
  @override
  DeviceType type() {
  ...
  void remoteControl(int index, int channel, int state) {
    if(Application.dataCenter.settingData.data.useYdl){
      String onoff = (state == 1 || state == 2) ? "true" : "false";
      Application.hardwareYDL.sendMessage('setsound:$id,$onoff,$index');
    }else{
      // 虽然提供了多channel支持,但是测试只需要1个
      Application.connection.sendMessage(
          Protocol.TopicsWriteAudio, json.encode(VibrateInfo(id: id, index: index, channel: channel, state: state)));
    }
  }
}
